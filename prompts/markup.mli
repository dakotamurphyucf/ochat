...
(** This function's type signature may look intimidating, but it is actually
    ...
    {[
      Element
        ("p" [ Text "HTML5 is "; Element ("em", [ Text "easy" ]); Text " to parse" ])
    ]}
...
val preprocess_input_stream
  :  (int, 's) stream
  -> (location * int, 's) stream * (unit -> location)
...
(** {2 Conformance status}
    ...
    {{:https://www.w3.org/TR/xml/} XML} and {{:https://www.w3.org/TR/xml-names/} Namespaces in XML} specifications.
