...
module Runtime_id = struct
  ...
  let is_valid value =
    ...
    && is_start value.[0]
    && String.for_all (String.drop_prefix value 1) ~f:is_rest
  ;;

  let of_string ?source value =
    if is_valid value
    then Ok value
    else
      Error
        (Diagnostic.error
           ?source
           ~code:"shell.invalid_runtime_id"
           "runtime ID must match [A-Za-z_][A-Za-z0-9_.:-]*")
  ;;

  let to_string t = t

  let qualify ~namespace t =
    if String.mem t ':'
    then t
    else Option.value_map namespace ~default:t ~f:(fun namespace -> namespace ^ ":" ^ t)
  ;;
end
