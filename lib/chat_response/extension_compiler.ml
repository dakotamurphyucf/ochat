...
let prepare ?(max_source_bytes = 256 * 1024) ~scripts ~capabilities (tool : Spec.tool) =
  ...
  in
  let%bind id, kind, surface, entrypoints, selected =
    match tool.implementation with
    | Moderator id when List.is_empty tool.uses ->
      Ok (id, Spec.Moderator_script, X.moderator_v1, X.moderator_entrypoints, capabilities)
    | Standalone { script; entrypoint = "run" } ->
      ...
      |> Result.map ~f:(fun selected ->
        script, Spec.Tool_script, X.tool_v1, X.tool_entrypoints, selected)
  ...
  in
  let%bind program =
    Chatml_host_runtime.compile_script ~surface ~required_bindings:entrypoints ~source ()
    |> Result.map_error ~f:(fun message ->
      [ D.error ~source:script.source_ref ~code:"chatml.invalid_handler" message ])
  in
  let fingerprint =
    [%sexp
      ("ochat.extension-compiler.v1" : string)
