  ...
  | Queued, _ :: _ | Ordinary _, _ ->
    ...
    let%bind claimed =
      claim
        ...
        (fun ~(executing : P.Moderator_execution.t)
          ~retirement_reason
          ~event:selected
          ~execute
          ~commit ->
           ...
           | Some _ ->
             let%bind expected =
               M.identity_snapshot manager |> Result.map_error ~f:failed
             in
             M.retire_queued_event_entries manager ~expected ~prepare:(fun ~snapshot ->
               let%map () =
                 commit
                   ~snapshot
                   ~requests:
             ...
             with_work
               (fun
                  ...
                  with_tools (fun on_tool_call ->
                    let session_id =
                      P.Id.Session.to_string executing.context.session_id
                    in
                    let now_ms =
