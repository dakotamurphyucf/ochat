...
let expect_string_array_arg (name : string) (value : Lang.value)
  ...
  match value with
  | Lang.VArray values ->
    Result.all (Array.to_list values |> List.map ~f:(expect_string_arg name))
    |> Result.map ~f:Array.of_list
  | _ -> Error (Printf.sprintf "%s: expected string array argument" name)
;;

let decode_pending_ui_request (eff : Lang.eff)
  : (pending_ui_request option, string) result
  =
  match eff.op with
  | "Approval.ask_text" ->
    (match eff.args with
...
let decode_ui_notify (args : Lang.value list) : (local_effect, string) result =
  ...
  | _ ->
    Error (Printf.sprintf "Ui.notify: expected 1 argument(s), got %d" (List.length args))
;;
...
let response_value_of_request (request : pending_ui_request) ~(response : string)
  : (Lang.value, string) result
  =
...
let handle_event
  ...
  else if Option.is_some session.current_exec
  then Error "Session is already handling an event."
  else (
