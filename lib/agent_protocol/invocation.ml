...
let validate_json ?(max_bytes = 8 * 1024 * 1024) ?(max_depth = 128) json =
  ...
  let charge bytes =
    ...
    if !remaining < 0 then invalid "invocation JSON exceeds byte limit" else Ok ()
  in
  let rec visit depth json =
    if depth > max_depth
    then invalid "invocation JSON exceeds depth limit"
    else (
        ...
        (match Float.of_string_opt value with
         | Some number when Float.is_finite number ->
           (try
            ...
            with
            | _ -> invalid "invalid JSON number")
         | _ -> invalid "invocation JSON requires finite numbers")
      | `Array values ->
      ...
      | `Object values ->
        ...
        List.fold_result values ~init:() ~f:(fun () (name, value) ->
          ...
          then invalid "duplicate invocation JSON field"
          else (
  ...
  then invalid "invocation JSON exceeds encoded byte limit"
  else Ok ()
;;
...
let closed fields allowed =
  ...
  with
  | None -> Ok ()
  | Some (key, _) -> invalid ("unknown invocation field: " ^ key)
;;
