...
module Glob = struct
  (* Convert a very small subset of gitignore globs ( *, ? ) to a
     anchored PCRE.  The conversion is deliberately conservative: if
     we encounter an unsupported pattern character we escape it so that
    ...
    let rec loop i =
      ...
      then ()
        ...
        begin
          ...
          | '?' -> Buffer.add_char buf '.'
          | '/' -> Buffer.add_char buf '/'
          | c when is_meta c -> Buffer.add_char buf '\\'; Buffer.add_char buf c
  ...
  let of_gitignore_line (line : string) : Pcre.regexp option =
    let line = String.strip line in
    if String.is_empty line || Char.equal line.[0] '#'
    then None
    else (
      let dir_pattern = String.is_suffix line ~suffix:"/" in
      Some (compile ~dir_pattern line))
end

let load_gitignore ~root : Pcre.regexp list =
  let path = Eio.Path.(root / ".gitignore") in
  match Or_error.try_with (fun () -> read_file_to_string path) with
  | Error _ -> []
  | Ok contents ->
    let lines = String.split_lines contents in
    List.filter_map lines ~f:Glob.of_gitignore_line

let fallback_blocklist : Pcre.regexp list =
  List.map
    [ "_build/"; "dist/"; "node_modules/"; ".git/"; ".hg/"; ".svn/" ]
    ~f:(fun pat -> Glob.compile ~dir_pattern:true pat)
...
let should_ignore ~regexps (rel_path : string) : bool =
  List.exists regexps ~f:(fun re -> Pcre.pmatch ~rex:re rel_path)
...
let crawl ~(root : _ Eio.Path.t) ~(f : doc_path:string -> markdown:string -> unit) : unit =
  let gitignore_res = load_gitignore ~root in
  let ignore_res = gitignore_res @ fallback_blocklist in
      ...
      (fun entry ->
         ...
         then ()
           ...
           | Ok stats -> (
               ...
               in
               if should_ignore ~regexps:ignore_res rel_path
               then ()
               else (
