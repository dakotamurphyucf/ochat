...
let get_vectors ~net docs =
  ...
  let slice_doc (id, doc, meta, len) =
    if len <= embedding_cap then [ id, doc, meta, len ]
    else (
      ...
      let rec loop cur_tokens cur_lines remaining acc slice_idx =
        ...
        | [] ->
          if cur_tokens = 0 then List.rev acc
          else
        ...
        | line :: rest ->
          ...
          if cur_tokens + t_line > window_tokens then
            ...
            loop (token_count (String.concat ~sep:"\n" overlap)) overlap (line :: rest)
              ((sid, slice_doc, meta, slen) :: acc) (slice_idx + 1)
          else
            loop (cur_tokens + t_line) (line :: cur_lines) rest acc slice_idx
      in
      loop 0 [] lines [] 0 )
  in
