...
let register ~dir:_ ~net : Gpt_function.t =
  ...
  let try_github_fast_path url : string option =
  ...
  in
  let fetch_and_convert url =
    (* 1. GitHub-optimised path *)
    match try_github_fast_path url with
    | Some md -> md
    | None ->
  ...
  in
  let run url =
    try Cache.find_or_add cache url ~ttl ~default:(fun () -> fetch_and_convert url) with
