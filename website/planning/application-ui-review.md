...
`recording.json` keeps captured requests/responses, the displayed final result, source hashes, runtime-source hashes, revision, timestamp, redaction description, and transport scope. The importer authenticates runtime hashes against the original recorded Git revision; missing historical sources or mismatched hashes reject. Later runtime edits/removals produce an explicit earlier-runtime label, without changing the capture or claiming current verification. Current example sources must still match. Changed final output, missing actual file-tool output, a specialist with tools, or inconsistent provider labeling reject. The downloadable `recorded-run.chatmd` preserves the redacted original transcript. Its Markdown hard breaks and indentation are intentional, so `.gitattributes` disables only end-of-line whitespace warnings for that single captured file.
