...
let markdown_and_html_test_engine () : Chat_tui.Highlight_tm_engine.t =
  let open Chat_tui in
  ...
  |> Jsonaf.parse
  |> Or_error.ok_exn
  |> Highlight_tm_loader.add_grammar_jsonaf registry
  |> Or_error.ok_exn;
  Highlight_tm_engine.create ~theme:Highlight_theme.github_dark
...
let%expect_test "fenced HTML uses its language grammar and preserves code literally" =
  let open Chat_tui in
  let hi_engine = markdown_and_html_test_engine () in
  let html = "<span>**literal markdown markers**</span>" in
  let text = String.concat ~sep:"\n" [ "```html"; html; "```" ] in
  let _, info =
    Highlight_tm_engine.highlight_text_with_info hi_engine ~lang:(Some "html") ~text:html
  in
  let rendered = render_message_to_string ~width:100 ~hi_engine ~text in
  printf "fallback=%s\n" (fallback_to_string info.fallback);
  printf
    "literal=%b fences=%b\n"
    (String.is_substring rendered ~substring:"**literal markdown markers**")
    (String.is_substring rendered ~substring:"```");
  [%expect
    {|
    fallback=None
    literal=true fences=false
    |}]
;;
