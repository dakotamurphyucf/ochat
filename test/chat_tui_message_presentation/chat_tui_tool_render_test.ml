...
let%expect_test "tool call rendering preserves call text" =
  let messages = [ "tool", "read_file({\"file\": \"lib/foo.ml\"})" ] in
  ...
  [%expect {| read_file({"file": "lib/foo.ml"}) |}]
;;
...
let%expect_test "apply_patch patch-only and adjacent status forms stay specialized" =
  let open Chat_tui in
  let render body =
    let tool_outputs = Hashtbl.create (module Int) in
    Hashtbl.set tool_outputs ~key:0 ~data:Types.Apply_patch;
    make_base_model ~messages:[ "tool_output", body ] ~tool_outputs
    |> render_to_string ~w:80 ~h:16
  in
  let patch =
    "*** Begin Patch\n*** Update File: lib/x.ml\n@@ let f\n-old\n+new\n*** End Patch"
  in
  let patch_only = render patch in
  let with_status = render ("Patch generated:\n" ^ patch) in
  let contains output text = String.is_substring output ~substring:text in
  print_s
    [%sexp
      (contains patch_only "*** Begin Patch" : bool)
    , (contains patch_only "+new" : bool)
    , (contains with_status "Patch generated:" : bool)
    , (contains with_status "*** Update File: lib/x.ml" : bool)];
...
let%expect_test "read_file output rendering preserves contents" =
  let open Chat_tui in
  let body =
    String.concat ~sep:"\n" [ "let x = 1"; "let y = 2"; "---"; "[File truncated]" ]
  in
  let messages = [ "tool_output", body ] in
  let tool_outputs = Hashtbl.create (module Int) in
