...
let parent_runtime ~env ~sw ~root =
  ...
  let host =
    AR.host
      ...
      ~cache_dir:root
      ~home:root
      ~session_id:"parent-native-owner"
...
let%expect_test
    "generated actor shares dispatch but preserves inherited roots, admission and own \
     moderator state"
  =
  List.iter [ `Allowed; `Plain; `Denied; `Revoked ] ~f:(fun mode ->
    with_actor_workspace (fun env workspace_instance ->
      Eio.Switch.run (fun sw ->
        ...
        in
        let artifact_store =
          Store.create ~env ~root:(Eio.Path.native_exn Eio.Path.(root / "artifacts"))
          |> store_ok
        in
        G.install ~artifact_store ~transaction_id:(P.Id.Transaction.create ()) definition
        |> Result.map_error ~f:(fun errors ->
          String.concat
            ~sep:"; "
        ...
        Exn.protect
          ...
          ~f:(fun () ->
            Eio.Time.with_timeout_exn (Eio.Stdenv.clock env) 15. (fun () ->
              ...
              |> protocol_ok
              |> ignore;
              let state = await_idle actor_value in
              ...
              in
              assert (
                not (String.is_substring rendered ~substring:"child-root-must-not-rebind"));
              let successes =
              ...
              (match mode with
               | `Allowed ->
                 ...
                 in
                 registry
                 := Chat_response.Generated_admission.capabilities (G.admission restored);
                 let rebuilt =
                   build
                 ...
                 Exn.protect ~finally:rebuilt.close ~f:(fun () ->
                   ...
                   [%test_eq: int] 2 !requests)
               | `Plain | `Denied | `Revoked -> ());
              print_s
                [%sexp
                  { mode : [ `Allowed | `Plain | `Denied | `Revoked ]
                  ; successes : int
