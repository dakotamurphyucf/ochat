...
let entries_of_history history =
  let allocator =
    History_entry.Allocator.create ~namespace:"tui-moderation-test" ~next_sequence:0
    |> ok_or_fail
  in
  let entries =
...
let model_of_history history =
  let _, entries = entries_of_history history in
  model_of_entries entries
;;

let runtime_with_moderator history manager =
  let allocator, entries = entries_of_history history in
  let moderator =
  ...
  in
  App_runtime.create
    ~model:(model_of_entries entries)
    ~moderator
...
let create_manager ?snapshot () =
  ...
  let allocator =
    History_entry.Allocator.create ~namespace:"tui-moderator-overlay" ~next_sequence:0
    |> ok_or_fail
  in
  ok_or_fail (Manager.create_entries ~artifact ~capabilities ~allocator ?snapshot ())
;;
...
let%expect_test "Ui.notify becomes a visible notice without mutating canonical history" =
  ...
  List.iter controller_outcome.system_notices ~f:(fun text ->
    ignore
      (App_runtime.add_system_notice_once runtime ~key:("system:" ^ text) text : bool));
...
let%expect_test "pending approval prompt is visible without mutating canonical history" =
  let manager =
    ok_or_fail
      (Manager.create_entries
         ...
         ~capabilities:Moderation.Capabilities.default
         ~allocator:
           (History_entry.Allocator.create ~namespace:"approval-test" ~next_sequence:0
            |> ok_or_fail)
         ())
  in
  ignore
  ...
  let runtime =
    App_runtime.create
      ~model
      ~moderator
      ~history_allocator:(Manager.history_allocator manager |> Option.value_exn)
      ()
  in
  ignore (App_runtime.refresh_messages runtime : Chat_tui.Model.projection_damage);
  print_s
    [%sexp
      (Option.map (App_runtime.pending_input runtime) ~f:App_runtime.render_pending_input
       : string option)];
  print_messages (Chat_tui.Model.messages model);
  print_s [%sexp (List.length (Chat_tui.Model.history_items model) : int)];
  [%expect
...
let%expect_test "runtime visible history reflects restored moderator snapshot" =
  ...
  let snapshot = ok_or_fail (Manager.identity_snapshot seeded_manager) in
  ...
  let runtime =
    App_runtime.create
      ~model:(model_of_history history)
      ~moderator
      ~history_allocator:(Manager.history_allocator resumed_manager |> Option.value_exn)
      ()
  in
  print_messages
    (App_runtime.visible_messages_of_history
       runtime
