...
let%expect_test
    "captured runtime construction installs owned lifecycle and moderator tools"
  =
  let module A = Agent_session.Session_actor in
  let module B = Agent_session.Runtime_builder in
  let module M = Chat_response.Moderator_manager in
  let module C = Chat_response.Tool_capability in
  List.iter [ `Foreground; `Idle; `Denied; `Revoked ] ~f:(fun mode ->
    with_actor_workspace (fun env workspace_instance ->
      Eio.Switch.run (fun sw ->
        let root =
        ...
        Exn.protect
          ...
          ~f:(fun () ->
            Eio.Time.with_timeout_exn (Eio.Stdenv.clock env) 15. (fun () ->
              ...
              assert (
                not (Agent_server.Runtime_owner.drain_idle_moderator owner |> protocol_ok));
              let names =
              ...
              assert_same_session_snapshot
                ...
                (Agent_session.Memory_backend.state backend);
              let snapshot =
                M.identity_snapshot (Option.value_exn runtime.moderator_manager)
                |> Result.ok_or_failwith
              in
              let count =
                match snapshot.current_state with
              ...
              (match mode with
               ...
               | _ -> assert (List.is_empty failures));
              print_s
                [%sexp
                  { mode : [ `Foreground | `Idle | `Denied | `Revoked ]
                  ; provider_calls = (!requests : int)
