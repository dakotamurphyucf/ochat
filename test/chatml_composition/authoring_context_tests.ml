...
let%expect_test
  ...
  List.iter
    ...
    ~f:(fun task ->
      ...
      List.iter
        [ "runtime.execution", "guide/chatml-execution-limits.md"
        ; "runtime.lifecycle", "guide/chatml-session-lifecycle.md"
        ]
        ~f:(fun (topic_id, path) ->
          let guidance =
            List.find_exn (items response) ~f:(fun item ->
              match Jsonaf.member "topic_id" item with
              | Some (`String id) -> String.equal id topic_id
              | _ -> false)
          in
          let reference =
            Authoring_sources.document installed ~path |> Result.ok_or_failwith
          in
          (* Installed rendering removes the final paragraph separator. *)
          [%test_eq: string]
            (String.rstrip reference.text)
            (field guidance "text" |> Jsonaf.string_exn));
      let task_guidance =
        List.find_exn (items response) ~f:(fun item ->
          match Jsonaf.member "topic_id" item with
