...
let%expect_test "Ctrl-Space on Agent does not mutate hidden Chat typeahead state" =
  let model =
    make_model
      ~input_line:"hidden chat draft"
      ~cursor_pos:17
  ...
  ignore
    (Chat_tui.Model.agent_call_started
     ...
     : bool);
  Chat_tui.Model.set_active_page model Chat_tui.Model.Page_id.Agent;
  let generation = Chat_tui.Model.typeahead_generation model in
  with_reducer ~model (fun ~runtime ~send_input ~send_internal:_ ~pump_until ~stop ->
    send_input (`Key (`ASCII ' ', [ `Ctrl ]));
    send_input (`Key (`Escape, []));
    pump_until (fun () ->
      Poly.(Chat_tui.Model.active_page model = Chat_tui.Model.Page_id.Chat));
    print_s
      [%sexp
        (Chat_tui.Model.typeahead_preview_open model : bool)
      , (Option.is_none (Chat_tui.Model.typeahead_completion model) : bool)
      , (Int.equal generation (Chat_tui.Model.typeahead_generation model) : bool)
      , (Option.is_none runtime.Chat_tui.App_runtime.typeahead_op : bool)
      , (runtime.Chat_tui.App_runtime.next_op_id : int)];
    ignore (stop () : bool));
  [%expect {| (false true true true 0) |}]
;;
