...
let%expect_test
    "moderator runtime suspends approval, blocks handle_event, and commits on resume"
  =
  let session =
  ...
  (match
   ...
   | Error msg -> print_endline ("error: " ^ msg));
  print_endline ("pending=" ^ show_pending_ui_request (Runtime.pending_ui_request session));
  print_endline ("state=" ^ show_value (Runtime.current_state session));
  print_endline ("queue=" ^ show_values (Runtime.queued_events session));
  print_endline ("effects=" ^ show_effects (Runtime.committed_local_effects session));
  ignore
  ...
  print_endline
    ("pending_after_resume="
     ^ show_pending_ui_request (Runtime.pending_ui_request session));
  ...
  print_endline
    ("effects_after_resume="
     ^ show_local_effects (Runtime.committed_local_effects session));
...
let%expect_test
    "moderator runtime Item helper builtins are available on the default surface"
  =
...
let%expect_test
    "moderator runtime Context helper builtins are available on the default surface"
  =
  let items =
    [| item ~id:"system-0" ~value:(message_item_value ~role:"system")
     ...
     ; item
         ...
         ~value:
           (json_object
              [ "type", json_string "function_call"; "name", json_string "search" ])
