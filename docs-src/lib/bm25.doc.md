...
Opaque handle of an index created by [`create`](#create).

### [`tokenize`](../../lib/bm25.mli)
...
* [`write_to_disk`](../../lib/bm25.mli) overwrites the given path with a
  single snapshot (mode `0o600`).
* [`read_from_disk`](../../lib/bm25.mli) reverses the operation.
...
1. **Stemming / lemmatisation** – Plug a stemmer in
   [`tokenize`](../../lib/bm25.ml) or run a pre-processing pass when building
   the corpus.
2. **Different k₁ / b values** – Modify
   `bm25_score` in [`bm25.ml`](../../lib/bm25.ml).
