...
1. Input buffers store grapheme-aligned byte offsets; `Input_display` translates
   valid UTF-8 prefixes into terminal-cell positions. Embedders must supply
   valid UTF-8 buffers. Rendering and editing boundaries are separate concerns;
   see [the editor contract](controller.doc.md#known-issues-and-limitations).
...
- `(cx, cy)` are the absolute screen coordinates of the caret inside the
  ...
  The active buffer uses byte offsets (`Model.cursor_pos` in Insert/Normal
  mode and `Model.cmdline_cursor` in Cmdline mode). `Input_display` converts
  valid UTF-8 prefixes to terminal-cell widths when positioning the caret.
