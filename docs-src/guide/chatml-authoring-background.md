...
let rec find_field fields name index =
  if index == Array.length(fields) then
    ...
    if entry.key == name then
      ...
      find_field(fields, name, index + 1)
...
let rec owner state job =
  match state with
  | `Empty -> `None
  | `Work(work, rest) ->
      if work.job == job then
...
let completion result =
  ...
  | `String("succeeded") ->
      Task.pure(`Succeeded(field(result, "value")))
  | `String("cancelled") ->
      Task.pure(`Cancelled(string(field(result, "reason"))))
  | `String("expired") ->
      Task.pure(`Expired)
  | `String("failed") ->
      let retryable =
        match field(result, "retryable") with
        | `Bool(value) -> value
        | _ -> false
      in
      Task.pure(`Failed({
...
let on_event ctx state event =
  ...
  | `Tool_invoked(p) ->
      ...
      Task.pure(`Work({
        job = job;
        invocation = p.context.invocation_id
      }, state))
