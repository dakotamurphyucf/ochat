...
document bodies or implementation code.
...
are separate semantic review obligations. `implementation_sources` supplies
coarse source-change detectors for the maintained semantic inventory; it does
not automatically discover rules or prove that their explanations are complete.
These hashes are embedded during the build and need no source files at runtime.
