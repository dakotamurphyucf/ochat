...
let speclist =
  [ "--root", Arg.Set_string root_dir, "Root directory to crawl recursively";
    ...
    "--desc", Arg.Set_string description, "One-line description (default: Markdown documentation index)";
...
let main env =
  ...
  if String.is_empty !root_dir then (
    Printf.eprintf "%s\n" usage;
    exit 1);
  ...
  Printf.printf "Markdown indexing completed. Index name: %s – stored under %s\n%!" !index_name
